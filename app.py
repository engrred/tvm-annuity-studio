...
with c1:
    R = st.number_input("Regular Payment (R)", value=None, placeholder="e.g. 10000", step=1000.0, format="%g")
with c2:
    i_in = st.number_input("Interest Rate (i)", value=None, placeholder="e.g. 0.08 or 8", step=0.01, format="%g")
    # Auto-convert kung sakaling 8 ang i-type sa halip na 0.08
    i = (i_in / 100.0 if i_in >= 1.0 else i_in) if i_in is not None else None
with c3:
    n = st.number_input("Total Payments (n)", value=None, placeholder="e.g. 5", step=1)
with c4:
    k = st.number_input("Full Delay (k)", value=None, placeholder="e.g. 2", step=1)

if st.button("Calculate All", type="primary"):
    if None in (R, i, n, k):
        st.warning("Paki-kumpleto muna ang lahat ng inputs bago mag-calculate, bro!")
    else:
